<script lang="ts">
  ...
  $effect(() => {
    ...
    const fetchStatus = async () => {
      const statusUrl = `/api/v1/spaces/${encodeURIComponent(space_id)}/status`;
  ...
  $effect(() => {
    ...
    const fetchHistory = async () => {
      isChartLoading = true;
      const historyUrl = `/api/v1/spaces/${encodeURIComponent(space_id)}/history?target_date=${encodeURIComponent(selectedDate)}`;
  ...
  $effect(() => {
    ...
    const fetchPeakData = async () => {
      ...
      const peaksUrl = `/api/v1/spaces/${encodeURIComponent(space_id)}/peaks?target_date=${encodeURIComponent(apiQueryDate)}`;
